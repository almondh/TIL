...
1.  추천 동영상 set은 사용자의 개인활동(watched, favorited, liked videos)를 기반으로 공동방문한 비디오그래프를 기반으로 확장
2.  동영상 set은 관련성/다양성을 위해 다양한 신호를 사용하여 순위를 매김
...
#### 2.5 User Interface
- 추천의 제시는 사용자 경험의 중요한 부분

#### 2.6 System Implementation(시스템 구현)
- 추천 : on-demand 계산 < batch-oriented 사전 계산
  - 장점 : 추천 생성 단계에서 CPU 리소스가 있는 대량의 데이터에 엑세스 + 사전에 생성된 추천 서빙을 짧은 지연으로 허용
  - 추천 사항을 파이프라인화, 데이터 세트를 하루에 여러번 업데이트하여 완화
- 유튜브의 추천 시스템
  1. 데이터 수집
    - raw 데이터 signal이 로그에 저장 -> 처리 -> 추출 -> 사용자 단위로 빅테이블에 저장
  2. 추천 생성
    - 추천사항 : 사용자/영상 그래프를 통해 수집 -> 맵리소스연산을 통해 추천 점수 -> 생성
  3. 추천 서비스



### 3. Evaluation
- A/B 테스트를 통한 live evaluation으로 추천시스템 평가
  - 실시간 트래픽이 그룹 구분지어줌
    1) 컨트롤/베이스
    2) 새로운 특성, 데이터, UI에 노출
  - 두 그룹은 미리 정의된 Metric set를 통해 서로 비교되며 요인을 제거하기 위해 다른 기간동안 교환 가능(?)
  - A/B 테스트의 장점
    - 실제 웹사이트의 UI 맥락에서 평가가 이루어짐
    - 여러가지 실험을 병행해서 실행, 빠른 피드백 가능
  - 단점
    - 모든 실험이 비교에 사용될 순 없음
    - 사용자그룹은 통계적으로 유의한결과를 얻기위한 충분한 트래픽이 필요
    - 주관적 목표의 평가는 사전에 정의된 Metrics 해석에 제한됨
- 서로 다른 측정 기준의 조합으로 평가
  - 주요지표
    1. CTR(Click Through Rate)
    2. long CTR(영상의 상당부분을 시청한 카운트 클릭수만)
    3. session 길이
    4. time until first long watch(처음으로 길게 시청할 때 까지의 시간)
    5. 추천 범위(추천으로 로그인 한 기록)
  - 이를 통해 실시간 트래픽의 시스템 변화를 평가 + 시스템 성능 지속적으로 추적
  
  
  
  ### 4. Results
  - 홈페이지의 60퍼 클릭이 추천! -> 추천시스템 성공시킴
  - 홈페이지의 추천시스템 성능과 비교해서 편향 존재 : 추천이 defalut 상단에 배치됨
    - 이를 조정하기 위해 "찾아보기" 페이지에서 CTR로 생성된 추천을, 다른 알고리즘으로 생성된 영상 set과 비교
      - (a:가장 많이 본 비디오, b:가장 많이 본 비디오), (b:가장 좋아하는 비디오, c:하루에 좋아요를 가장 많이받은 비디오)
